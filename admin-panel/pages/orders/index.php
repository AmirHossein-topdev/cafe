...
// پارامترهای ورودی را دریافت می‌کنیم و معتبر بودن‌شان را چک می‌کنیم
...
// حذف سفارش
if (getParam('action') === 'delete' && getParam('id') && getParam('table')) {
    $postId = (int) getParam('id');
    $table = preg_replace('/[^a-zA-Z0-9_]/', '', getParam('table')); // فقط حروف، اعداد و _ برای امنیت
...
if (getParam('action') === 'update_all_status' && getParam('table') && getParam('status') !== null) {
    $table = preg_replace('/[^a-zA-Z0-9_]/', '', getParam('table'));
    $status = getParam('status') == '1' ? 1 : 0;

    try {
        $query = $db->prepare("UPDATE $table SET status = :status");
...
// حذف همه اقلام
if (getParam('action') === 'delete_all' && getParam('table')) {
    $table = preg_replace('/[^a-zA-Z0-9_]/', '', getParam('table'));
    ...
    try {
        ...
        echo 'خطا در حذف همه اقلام: ' . $e->getMessage();
    }
}
